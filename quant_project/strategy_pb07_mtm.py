"""
PB07 Q1 Mean Reversion Strategy - Daily Mark-to-Market Implementation.
...
class PB07DailyMarkToMarket:
    """PB07 Q1 mean reversion strategy with daily mark-to-market."""
    ...
    def __init__(self, signals_path, price_path):
        ...
        print(f"PB07 Q1 Daily Mark-to-Market Strategy")
        print(f"Data: {len(self.data)} rows, {self.data['date'].min().date()} to {self.data['date'].max().date()}")
    
    def find_entries_exits(self, horizon=20):
        """Find all PB07 Q1 signal events."""
        print(f"\nFinding PB07 Q1 entries (20-day holding)...")
        
        pb07 = self.data['PB07'].values
        
        # Create quintiles
        pb07_valid = pb07[~np.isnan(pb07)]
        q1_threshold = np.percentile(pb07_valid, 20)  # Bottom 20%
        
        pb07_signal = (pb07 <= q1_threshold).astype(int)
        
        all_events = []
        for i in range(1, len(self.data)):
            if pb07_signal[i-1] == 1:
                all_events.append(i)
        
        print(f"Total PB07 Q1 signal events: {len(all_events)}")
    ...
    def save_results(self):
        """Save results."""
        self.daily_returns_df.to_csv('research_output/strategy_pb07_daily_mtm.csv', index=False)
        ...
        trades_df.to_csv('research_output/strategy_pb07_trades_mtm.csv', index=False)
        
        with open('research_output/strategy_pb07_metrics_mtm.json', 'w') as f:
            json.dump(self.metrics, f, indent=2)
        
        print(f"\nResults saved:")
        print(f"  strategy_pb07_daily_mtm.csv ({len(self.daily_returns_df)} rows)")
        print(f"  strategy_pb07_trades_mtm.csv ({len(self.trades)} trades)")
        print(f"  strategy_pb07_metrics_mtm.json")
    
    def print_summary(self):
        """Print summary."""
        m = self.metrics
        print(f"\n{'='*60}")
        print("PB07 SUMMARY (Daily Mark-to-Market)")
        print(f"{'='*60}")
...
def main():
    strategy = PB07DailyMarkToMarket('signals_cleaned.csv', 'price_cleaned.csv')
